...
public class LocaleEdit extends AppCompatActivity {
    ...
    protected void onCreate(Bundle paramBundle) {
        ...
        BundleScrubber.scrub(getIntent().getBundleExtra(
                dev.ukanth.ufirewall.locale.Intent.EXTRA_BUNDLE));
    ...
    public void finish() {
        if (mIsCancelled) {
            ...
            if (!G.isProfileMigrated()) {
                ...
                resultIntent.putExtra(dev.ukanth.ufirewall.locale.Intent.EXTRA_BUNDLE, PluginBundleManager.generateBundle(getApplicationContext(), idx + "::" + action));
            } else {
                int idx = group.indexOfChild(radioButton);
                resultIntent.putExtra(dev.ukanth.ufirewall.locale.Intent.EXTRA_BUNDLE, PluginBundleManager.generateBundle(getApplicationContext(), idx + "::" + action));
            }
            resultIntent.putExtra(dev.ukanth.ufirewall.locale.Intent.EXTRA_STRING_BLURB, action);
